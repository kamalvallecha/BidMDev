...
const InvoiceEdit = () => {
  ...
  useEffect(() => {
    if (bidId && partners.length > 0 && activeLOI !== null) {
      // Get the current partner-LOI combination key
      const currentKey = `${partners[activeVendor]}_${activeLOI}`;
      
      // Load saved invoice details for this partner-LOI combination
      const savedDetails = partnerInvoiceDetails[currentKey];
      if (savedDetails) {
        setFormData({
          poNumber: savedDetails.poNumber || "",
          invoiceDate: savedDetails.invoiceDate || "",
          invoiceSent: savedDetails.invoiceSent || "",
          invoiceSerial: savedDetails.invoiceSerial || "",
          invoiceNumber: savedDetails.invoiceNumber || "",
          invoiceAmount: savedDetails.invoiceAmount || "0.00",
        });
      }
  ...
  }, [bidId, activeVendor, activeLOI, partners, partnerInvoiceDetails]);
  ...
  useEffect(() => {
    ...
    if (
      savedState &&
      (!invoiceData.deliverables.length || !formData.invoiceDate)
    ) {
      const {
        formData: savedForm,
  ...
  const fetchPartnerData = async () => {
    try {
      const response = await axios.get(`/api/invoice/${bidId}/partner-data`);
      const data = response.data;

      // Extract unique partners and LOIs from partner_data keys
      const uniquePartners = [
        ...new Set(
          Object.keys(data.partner_data).map((key) => key.split("_")[0]),
        ),
      ];
      const uniqueLois = [
        ...new Set(
          Object.keys(data.partner_data).map((key) =>
  ...
  const fetchInvoiceData = async () => {
    try {
      const response = await fetch(
        `/api/invoice/${bidId}/${partners[activeVendor]}/${activeLOI}/details`,
      );
      ...
      const hasDeliveries = data.deliverables.some(
        (item) => item.nDelivered > 0,
      );
  ...
  const handleSubmit = async () => {
    try {
      ...
      const currentDeliverables = invoiceData.deliverables.filter(
        (item) => item.nDelivered > 0,
      );
      const hasInvalidCurrentCPI = currentDeliverables.some(
        (item) =>
          !item.finalCPI ||
      ...
      if (hasInvalidCurrentCPI) {
        alert(
          `Please fill in all Final CPI values for ${partners[activeVendor]} with ${activeLOI} min LOI`,
        );
        return;
      }
      ...
      for (
        ...
        for (const loi of lois) {
          ...
          const response = await fetch(
            `/api/invoice/${bidId}/${currentPartner}/${loi}/details`,
          );
          const data = await response.json();

          if (data.deliverables && data.deliverables.length > 0) {
            const deliveredItems = data.deliverables.filter(
              (item) => item.nDelivered > 0,
            );

            if (deliveredItems.length > 0) {
              const hasInvalidCPI = deliveredItems.some(
                (item) =>
                  !item.finalCPI ||
                  parseFloat(item.finalCPI) === 0 ||
                  item.finalCPI === "0" ||
              ...
              if (hasInvalidCPI) {
                alert(
                  `Please check ${currentPartner} with ${loi} min LOI for Final CPI`,
                );
                return;
              }
            }
      ...
      // Submit the invoice
      const submitResponse = await fetch(`/api/invoice/${bidId}/submit`, {
        method: "POST",
        headers: {
  ...
  const handleSave = async () => {
    try {
      ...
      const response = await fetch(`/api/invoice/${bidId}/save`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
      ...
      alert(
        `Data saved successfully for ${currentPartner} with ${activeLOI} min LOI!`,
      );
    } catch (error) {
  ...
  const renderAudienceTable = (audienceId) => {
    const audienceDeliverables = invoiceData.deliverables.filter(
      (item) => item.audience_id === audienceId
    );

    if (!audienceDeliverables.length) return null;

    // Get the first deliverable to access audience details
    const audienceDetails = audienceDeliverables[0];

    return (
      <TableContainer component={Paper} sx={{ mb: 4 }} key={audienceId}>
        <Typography variant="h6" sx={{ p: 2, fontWeight: 'bold' }}>
          Audience: {audienceDetails.ta_category} - {audienceDetails.broader_category} - {audienceDetails.mode} - IR {audienceDetails.ir}%
        </Typography>
        <Table>
          <TableHead>
            <TableRow>
  ...
  return (
    <Box sx={{ p: 3 }}>
      ...
      <Tabs value={activeVendor} onChange={handlePartnerChange} sx={{ mb: 3 }}>
